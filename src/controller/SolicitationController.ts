...
import { ICreateSolicitation, ICreateSolicitationExhibitor, IUpdateSolicitation } from "../helpers/dto"; 
...
export class SolicitationController {
    ...
    async createSolicitationExhibitor(request: Request, response: Response): Promise<Response> {
        try{
            const  { detail, email, eventId, name, phone, workedInThePast }: ICreateSolicitationExhibitor = request.body;
            const solicitation = await solicitationService.createSolicitationExhibitor({detail, email, eventId, name, phone, workedInThePast})
            return response.status(201).send(solicitation);
        }catch(err){
            if(err instanceof AppError){
                return response.status(err.statusCode).json({error: err.message});
